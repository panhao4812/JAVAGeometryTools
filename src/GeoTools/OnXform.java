...
public OnXform(){this.Zero();this.m_xform[3][3] = 1;}
	public void Zero()
	{
	 for (int i=0;i<4;i++){
		 for (int j=0;j<4;j++){
	...
	{
	  this.Rotation( (float)Math.sin(angle),(float)Math.cos(angle), axis, center );
	}
public void Rotation(On3dVector start_dir,On3dVector end_dir,On3dPoint rotation_center){
			    start_dir.Unitize();
			    end_dir.Unitize();
	...
	void Rotation(  float sin_angle, float cos_angle,On3dVector axis,On3dPoint center)
	{
	  Identity();
	  if ( Math.abs(sin_angle) >= 1.0f-Float.MIN_VALUE&& Math.abs(cos_angle) <= Float.MIN_VALUE )
	  {
	    cos_angle = 0.0f;
	    if ( sin_angle < 0.0f ) sin_angle = -1.0f; else sin_angle = 1.0f;
	  }
	  else if ( Math.abs(cos_angle) >= 1.0f-Float.MIN_VALUE && Math.abs(sin_angle) <= Float.MIN_VALUE )
	  {
	    sin_angle = 0.0f;
	    if ( cos_angle < 0.0f ) cos_angle = -1.0f; else cos_angle = 1.0f;
	  }
	  else if ( Math.abs(cos_angle*cos_angle + sin_angle*sin_angle - 1.0f) > Float.MIN_VALUE )
	  {
	  ...
	  if (sin_angle != 0.0f || cos_angle != 1.0f) 
	  {
	    ...
	    if ( center.x != 0.0 || center.y != 0.0 || center.z != 0.0 ) {
	      m_xform[0][3] = -((m_xform[0][0]-1.0f)*center.x + m_xform[0][1]*center.y + m_xform[0][2]*center.z);
	      m_xform[1][3] = -(m_xform[1][0]*center.x + (m_xform[1][1]-1.0f)*center.y + m_xform[1][2]*center.z);
	      m_xform[2][3] = -(m_xform[2][0]*center.x + m_xform[2][1]*center.y + (m_xform[2][2]-1.0f)*center.z);
	    }
	...
	public static OnXform XformMul(OnXform xf1,OnXform rhs )
	{
	  ...
	  m[0][0] = xf1.m_xform[0][0]*p[0] + xf1.m_xform[0][1]*p[4] + xf1.m_xform[0][2]*p[8] + xf1.m_xform[0][3]*p[12];
	  m[0][1] = xf1.m_xform[0][0]*p[1] + xf1.m_xform[0][1]*p[5] + xf1.m_xform[0][2]*p[9] + xf1.m_xform[0][3]*p[13];
	  m[0][2] = xf1.m_xform[0][0]*p[2] + xf1.m_xform[0][1]*p[6] + xf1.m_xform[0][2]*p[10] + xf1.m_xform[0][3]*p[14];
	  m[0][3] = xf1.m_xform[0][0]*p[3] + xf1.m_xform[0][1]*p[7] + xf1.m_xform[0][2]*p[11] + xf1.m_xform[0][3]*p[15];

	  m[1][0] = xf1.m_xform[1][0]*p[0] + xf1.m_xform[1][1]*p[4] + xf1.m_xform[1][2]*p[ 8] + xf1.m_xform[1][3]*p[12];
	  m[1][1] = xf1.m_xform[1][0]*p[1] + xf1.m_xform[1][1]*p[5] + xf1.m_xform[1][2]*p[ 9] + xf1.m_xform[1][3]*p[13];
	  m[1][2] = xf1.m_xform[1][0]*p[2] + xf1.m_xform[1][1]*p[6] + xf1.m_xform[1][2]*p[10] + xf1.m_xform[1][3]*p[14];
	  m[1][3] = xf1.m_xform[1][0]*p[3] + xf1.m_xform[1][1]*p[7] + xf1.m_xform[1][2]*p[11] + xf1.m_xform[1][3]*p[15];

	  m[2][0] = xf1.m_xform[2][0]*p[0] + xf1.m_xform[2][1]*p[4] + xf1.m_xform[2][2]*p[ 8] + xf1.m_xform[2][3]*p[12];
	  m[2][1] = xf1.m_xform[2][0]*p[1] + xf1.m_xform[2][1]*p[5] +xf1. m_xform[2][2]*p[ 9] + xf1.m_xform[2][3]*p[13];
	  m[2][2] = xf1.m_xform[2][0]*p[2] + xf1.m_xform[2][1]*p[6] + xf1.m_xform[2][2]*p[10] + xf1.m_xform[2][3]*p[14];
	  m[2][3] = xf1.m_xform[2][0]*p[3] + xf1.m_xform[2][1]*p[7] + xf1.m_xform[2][2]*p[11] + xf1.m_xform[2][3]*p[15];

	  m[3][0] = xf1.m_xform[3][0]*p[0] + xf1.m_xform[3][1]*p[4] + xf1.m_xform[3][2]*p[ 8] + xf1.m_xform[3][3]*p[12];
	  m[3][1] = xf1.m_xform[3][0]*p[1] + xf1.m_xform[3][1]*p[5] + xf1.m_xform[3][2]*p[ 9] + xf1.m_xform[3][3]*p[13];
	  m[3][2] = xf1.m_xform[3][0]*p[2] + xf1.m_xform[3][1]*p[6] + xf1.m_xform[3][2]*p[10] + xf1.m_xform[3][3]*p[14];
	  m[3][3] = xf1.m_xform[3][0]*p[3] + xf1.m_xform[3][1]*p[7] + xf1.m_xform[3][2]*p[11] + xf1.m_xform[3][3]*p[15];
	...
	public boolean ChangeBasis(         
			OnPlane plane0, // initial plane
	...
	public boolean  ChangeBasis(
			  On3dVector  X0,On3dVector  Y0, On3dVector  Z0,
			  On3dVector  X1,On3dVector  Y1, On3dVector  Z1)
			{
			  ...
			  float R[][] = {{On3dVector.OnDotProduct (X1,X1),a,b,On3dVector.OnDotProduct (X1,X0), On3dVector.OnDotProduct (X1,Y0), On3dVector.OnDotProduct (X1,Z0)},
			                    {a,On3dVector.OnDotProduct (Y1,Y1),c,On3dVector.OnDotProduct (Y1,X0),On3dVector.OnDotProduct (Y1,Y0),On3dVector.OnDotProduct (Y1,Z0)},
			                    {b,c,On3dVector.OnDotProduct (Z1,Z1),On3dVector.OnDotProduct (Z1,X0),On3dVector.OnDotProduct (Z1,Y0),On3dVector.OnDotProduct (Z1,Z0)}};
	...
	public boolean ChangeBasis(
			  On3dPoint   P0,  On3dVector  X0,On3dVector  Y0,On3dVector  Z0, 
			  On3dPoint   P1, On3dVector  X1, On3dVector  Y1,On3dVector  Z1  )
			{
			  boolean rc = false;
			  ...
			  rc = CB.ChangeBasis(new On3dVector(1,0,0),new On3dVector(0,1,0), new On3dVector(0,0,1),X1,Y1,Z1);		
			  CB.mul(T1);
			  CB.mul(F0);
	...
	public static  void DrawGrid(PApplet p) {
		  ...
		  for (int x = -64; x < 65; x ++) {
		    ...
		    if(x==0){p.line(0,0,0,-254,0,0);}else{
		    	p.line(x * gridSize, -64 * gridSize,0, x * gridSize, 64 * gridSize,0);}
		  }
		  ...
		  for (int y = -64; y < 65; y ++) {
		    ...
		    if(y==0){p.line(0,0,0,0,-254,0);}else{
		    	p.line(-64 * gridSize, y * gridSize, 0,64 * gridSize, y * gridSize,0);}
		  }	
		// strokeWeight(1f);
		  p.stroke(255,0,0);
